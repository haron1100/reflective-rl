...
def gen_reflections_k(model, tok, refl_prompt: str, k: int, max_new_tokens: int, temperature: float):
    """
    Generate K reflections for the same prompt in a single call.
    Returns List[str] of reflection texts.
    """
    # One prompt, multiple returns
    inputs = tok(refl_prompt, return_tensors="pt").to(model.device)
    ...
    gc = GenerationConfig(
        ...
        output_scores=False,  # we don't need scores for reflections
        use_cache=True,
    )
    out = model.generate(
        **inputs,
    ...
    seqs = out.sequences  # [k, prompt_len + gen_len]
    texts = []
    for i in range(seqs.size(0)):
        tail = seqs[i, prompt_len:]
        texts.append(tok.decode(tail, skip_special_tokens=True))
    return texts


@torch.inference_mode()
def gen_codes_batch_with_scores(model, tok, retry_prompts, max_new_tokens: int, temperature: float):
    """
    Batched code generation for reflection-conditioned retries.
    Returns:
      - code_texts: List[str] decoded generated tails (no prompt)
      - teacher_scores_list: List[List[Tensor[V]]] per sample (T_i steps with [1,V] logits each)
      - gen_ids_list: List[LongTensor[1, T_i]] token ids of generated tails
    """
    B = len(retry_prompts)
    inputs = tok(retry_prompts, return_tensors="pt", padding=True, truncation=False).to(model.device)
    ...
    code_texts, teacher_scores_list, gen_ids_list = [], [], []
    # out.scores is a list[T_max] of [B, V] logits
    T_max = len(out.scores)
    for i in range(B):
        gen_ids = seqs[i, prompt_lens[i].item():]
        gen_ids_list.append(gen_ids.unsqueeze(0))
        code_texts.append(tok.decode(gen_ids, skip_special_tokens=True))
        T_i = gen_ids.size(0)
        # collect per-step logits for this sample (only first T_i steps)
        per_steps = []
        for t in range(T_i):
            per_steps.append(out.scores[t][i].unsqueeze(0))  # shape [1, V], matches collect_teacher_topk expectation
        teacher_scores_list.append(per_steps)
    return code_texts, teacher_scores_list, gen_ids_list
...
def main():
    ...
    # speed knobs
    ap.add_argument("--use_bettertransformer", action="store_true", help="enable BetterTransformer fastpath if available")
    ap.add_argument("--max_new_tokens_code", type=int, default=None, help="override cfg.max_new_tokens_code")
    ap.add_argument("--max_new_tokens_refl", type=int, default=None, help="override cfg.max_new_tokens_refl")
    ap.add_argument("--code_temperature", type=float, default=None, help="override cfg.temp_code")
    ap.add_argument("--refl_temperature", type=float, default=None, help="override cfg.temp_refl")
    args = ap.parse_args()
    ...
    # optional CLI overrides for speed
    if args.max_new_tokens_code is not None:
        cfg.max_new_tokens_code = args.max_new_tokens_code
    if args.max_new_tokens_refl is not None:
        cfg.max_new_tokens_refl = args.max_new_tokens_refl
    if args.code_temperature is not None:
        cfg.temp_code = args.code_temperature
    if args.refl_temperature is not None:
        cfg.temp_refl = args.refl_temperature
    ...
    if write_header:
        csv_w.writerow(
            ["step", "pid", "R0", "R_best", "A_best", "KD", "refl_loss", "kd_loss", "sft_loss",
    ...
    for idx, pb in pbar:
        ...
        model.set_adapter("default")
        base_prompt = pb.prompt + "\n# Write the function above."
        # Greedy by design for baseline reproducibility
        out = model.generate(
            **tok(base_prompt, return_tensors="pt").to(model.device),
            do_sample=False,
            max_new_tokens=cfg.max_new_tokens_code,
            eos_token_id=tok.eos_token_id,
            pad_token_id=tok.pad_token_id,
        )
        base_code = tok.decode(out.sequences[0], skip_special_tokens=True)
        R0, _ = eval_code(base_code, pb.tests)

        # 2) Reflections (batched) & code retries (batched)
        model.set_adapter("reflector")
        refl_prompt = format_reflection_prompt(pb.prompt, R0)
        reflections = gen_reflections_k(
            model, tok, refl_prompt,
            k=cfg.k_reflections,
            max_new_tokens=cfg.max_new_tokens_refl,
            temperature=cfg.temp_refl
        )

        # Build retry prompts
        retry_prompts = [format_retry_prompt(pb.prompt, r) for r in reflections]

        # Teacher runs (batched) under student adapter (same base weights)
        model.set_adapter("default")
        code_texts, teacher_scores_list, gen_ids_list = gen_codes_batch_with_scores(
            model, tok, retry_prompts,
            max_new_tokens=cfg.max_new_tokens_code,
            temperature=cfg.temp_code
        )

        # Evaluate and collect candidates
        cand = []
        for r_text, code_text, scores, gen_ids in zip(reflections, code_texts, teacher_scores_list, gen_ids_list):
            R_i, _ = eval_code(code_text, pb.tests)
            A_i = R_i - R0
            cand.append((A_i, r_text, code_text, scores, gen_ids))
        ...
        if best and best[0] >= cfg.tau_gate:
            ...
            if gen_ids is not None and gen_ids.numel() > 0 and len(teacher_scores) > 0:
                # reshape teacher_scores (list[T_i] of [1, V]) -> collect top-k per step
                teacher_topk = []
                for step_logits in teacher_scores:
                    # mimic `collect_teacher_topk` input: a list of [B,V] tensors
                    teacher_topk.append(step_logits)  # we will call top-k later directly
                # Convert to top-k distributions
                # `collect_teacher_topk` expects raw [B,V] scores list; reuse it:
        ...
        R_best_num = R0 + (A_best if cand else 0.0)
        if A_best > 0:
            refl_help += 1
        if did_kd:
            kd_yes += 1
        ...
        # CSV row
        refl_len = len((cand[0][1] if cand else "").split())
        code_len = len((best[2] if best else base_code).split()) if cand else len(base_code.split())
        secs = time.time() - t0
        csv_w.writerow([
